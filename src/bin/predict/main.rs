...
fn do_main(options: Options) -> Result<(), ()> {
    ...
    let thetas: ThetaFile = serde_yaml::from_str(&file_contents)
        .map_err(|e| tracing::error!("Error parsing config file: {e}"))?;

    let rdr = Reader::from_path(config.path_to_test_file)
    ...
    let mut linear_regression = ft_linear_regression::linear_regression::LinearRegression::new(
        data.clone(),
        None,
        0,
        config.graph_path,
